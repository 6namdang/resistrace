...
type NcbiGenomeSearchProps = {
  selectedGenome: NcbiGenomeSequence | null
  onGenomeChange: (genome: NcbiGenomeSequence | null) => void
}
  ...
  return (
    <Card className="rounded-lg shadow-none">
      <CardHeader className="border-b border-border/70">
        ...
        <CardDescription>
          Find complete genomic records by organism name.
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        {selectedGenome ? (
          <div className="flex items-start justify-between gap-3 rounded-md border border-primary/30 bg-accent/40 p-3">
            <div className="flex min-w-0 gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-xs text-muted-foreground">
                  {selectedGenome.header}
                </p>
                <p className="mt-1 text-xs font-medium text-primary">
                  {baseCountFormatter.format(selectedGenome.length)} bp loaded
                  in memory
                </p>
              </div>
            </div>
            <Button
              ...
              variant="ghost"
              size="icon-sm"
              onClick={() => onGenomeChange(null)}
              aria-label="Remove selected NCBI genome"
            >
              <X />
            </Button>
          </div>
        ...
        {results && results.length > 0 ? (
          <div className="divide-y divide-border border-y border-border">
            {results.map((result) => {
              const isSelected = selectedGenome?.uid === result.uid
